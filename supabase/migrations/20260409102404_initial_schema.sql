-- ============================================
-- Complete Quiz Database Setup for Supabase
-- ============================================
...
CREATE TABLE IF NOT EXISTS quizzes (
  ...
  pin_code TEXT UNIQUE, -- 6-digit code for joining
...
CREATE POLICY "Users can view questions for public quizzes" ON questions
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM quizzes 
      WHERE quizzes.id = questions.quiz_id 
...
CREATE POLICY "Users can manage answers for own quiz questions" ON answers
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM questions q
      JOIN quizzes quiz ON q.quiz_id = quiz.id
      WHERE q.id = answers.question_id 
      AND quiz.creator_id = auth.uid()
    )
  );

CREATE POLICY "Users can view answers for public quiz questions" ON answers
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM questions q
      JOIN quizzes quiz ON q.quiz_id = quiz.id
...
CREATE POLICY "Users can view attempts for their quizzes" ON quiz_attempts
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Users can view results for their quizzes" ON quiz_results
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM quiz_sessions qs
      JOIN quizzes q ON qs.quiz_id = q.id
